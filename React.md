...
* \[categoryId] - `useEffect` отслеживает изменение `categoryId` и делает запрос, если изменение проиозошло.
...
# УРОК 9 Делаем функционал сортировки и фильтрации пицц. Redux Toolkit 
...
Redux позволяет точечно выполнять перерисовку (не перерисовывая весь/все компонент)
